...
############################################  \/     \/     \/     \/     \/     \/   ############################################
##################################################################################################################################
########################################## Type of beam elements: Euler-Bernoulli Beam ###########################################
##################################################################################################################################
...
def main():
    ...
    thetas = np.array([30])      # Cell Angle  
    elements_per_line = 10      # Number of elements per beam
    
    ## Define material constants and initial parameters
    E = 210e3                                                           # Young's modulus (N/mm^2)
    nu = 0.3                                                            # Poisson's ratio
    beta = 1/15                                                         # Slenderness factor
    b = beta*L                                                          # Breadth (mm)      |- Details of the
    h = beta*L                                                          # Depth (mm)        |- beam's cross-section
    A = b*h                                                             # Area of cross-section (mm^2)
    I = (b*(h**3))/12                                                   # Area moment of inertia (mm^4)
    ngp = 2                                                             # Number of Gauss points for numerical integration (linear)
    rho = 7850                                                          # Mass density (kg/m^3)
    omega0 = 1e6*((np.pi/L)**2)*np.sqrt((E*I)/(rho*A))                  # First flexural natural frequency (for non-dimensionalizing eigen frequencies)
    omeg_mult = 10                                                      # Maximum multiple of omega0 for display of dispersion diagram and transmission loss diagram 
    for mesh_itr in range(len(thetas)):
        theta = math.radians(thetas[mesh_itr])
        create_mesh(L, theta, elements_per_line)
        
        conn_mat = np.loadtxt(os.path.join('Data Files', 'conn_mat.dat'), dtype = int)-1        # Connectivity matrix (contains indices, force data type to int)
        coordinates = np.loadtxt(os.path.join('Data Files', 'coordinates.dat'))                 # Coordinates --> node information  
        
        const_param = np.array([E, nu, A, I, ngp, rho, omega0, L, theta, omeg_mult])   
        analysis(const_param, conn_mat, coordinates, theta, thetas[mesh_itr])

# Function: Create unit cell and mesh
def create_mesh(L, theta, elements_per_line):
    ...
    coords = []            # ordered list of node (x, y)
    conn = []              # element connectivity as (n1, n2)
    coord_to_tag = {}      # maps (x, y, z) → Gmsh point tag
    tag_to_index = {}      # maps Gmsh point tag → sequential node number

    # === Helper: Add point if not already added ===
    def add_point(pt):
        if pt not in coord_to_tag:
            tag = gmsh.model.geo.addPoint(*pt, 0)
            coord_to_tag[pt] = tag
            coords.append((pt[0], pt[1]))
        return coord_to_tag[pt]  # tag (not the node index)

    # === Helper: Create points along a beam ===
    ...
    beam1 = linspace_points(p1, p2, elements_per_line)          # === Beam 1: p1 → p2 ===
    beam2 = linspace_points(p2, p4, elements_per_line)[1:]      # === Beam 2: p2 → p4 === 
    beam3 = linspace_points(p2, p3, elements_per_line)[1:]      # === Beam 3: p2 → p3 ===
    
    # Add points and lines for vertical beam (beam 1)
    ...
    # Alternate adding points from beam2 and beam3 completely
    b2_idx, b3_idx = 0, 0
    len_b2, len_b3 = len(beam2), len(beam3)
    turn = 0  # 0 for beam2, 1 for beam3
    ...
    tag_to_index = {tag: i+1 for i, tag in enumerate(tag_list)}  # 1-based

    # === Write coordinates.dat ===
    with open(os.path.join("Data Files", "coordinates.dat"), "w") as f:
        for tag in tag_list:
            x, y, _ = gmsh.model.getValue(0, tag, [])
            f.write(f"{x:.6f}\t{y:.6f}\n")

    # === Write conn_mat.dat ===
    with open(os.path.join("Data Files", "conn_mat.dat"), "w") as f:
        for n1, n2 in conn:
